...
pub fn explicabilite(plan: Vec<Op>, ground: &GroundProblem) -> Vec<Necessaire> {
    ...
    for a in 1..=length {
        ...
        for nec in &out {
            ...
            if nec.presence(resumei) {
                //list.contains(resumei)
                //besoin de l'action
        ...
        if presenceaction == false {
            newout.push(newnecess(resumei));
        }
        out = newout.clone();
    }
        ...
        if preced == false {
            ...
            for pre in precon {
                for eff in effet {
                    if pre.var() == eff.var() {
                        let ot =
                            newot(*plan3.get(u - 1).unwrap(), i - 1, *plan3.get(u).unwrap(), i);
                        out.push(ot);
                    }
                }
            }
...
pub fn xdijkstra(plan: Vec<Op>, ground: &GroundProblem) -> Vec<Necessaire> {
    ...
    write!(
        output,
        "Explication des liens de support et de leur Nécessité \n"
    )
    .expect("Something went wrong writing the file");
    ...
    for t in &traite {
        if t.nec() {
            if t.chemin().is_none() {
                write!(
                    output,
                    "l'étape {} est nécessaire car elle accomplis un but \n",
                    t.opnec().numero()
                )
                .expect("Something went wrong writing the file");
            } else {
...
*******/
pub fn explicationsupport(
    plan: &Vec<Op>,
    support: &DMatrix<i32>,
    /*ground : &GroundProblem,*/ step1: i32,
    step2: i32,
) -> Necessaire {
...
pub fn explicationmenace(
    ...
    if !m.is_none() {
        if *m.unwrap() == 0 {
            println!(
                " L'étape {} n'est pas une menace pour l'étape {}",
                step1, step2
            );
        } else if *m.unwrap() == 1 {
            println!(
                " L'étape {} est une menace pour l'étape {} et doit être placer après",
                step1, step2
            );
        } else if *m.unwrap() == -1 {
            println!(
                " L'étape {} est une menace pour l'étape {} et doit être placer avant",
                step1, step2
            );
        } else if *m.unwrap() == -2 {
...
pub fn explicationmenacequestiondetail(
    plan: &Vec<Op>,
    menace: &DMatrix<i32>,
    support: &DMatrix<i32>,
    /*ground : &GroundProblem,*/ step1: i32,
...
ATTENTION Step 1 est l'étape supporté et step 2 la supportante
*******/
pub fn supportindirectavantagepoid(
    step1: i32,
    step2: i32,
    plan: &Vec<Op>,
    ground: &GroundProblem,
...
pub fn synchronisation(
    h: &HashMap<SymId, Vec<Op>>,
    support: &DMatrix<i32>,
    plan: &Vec<Op>, /*ground: &GroundProblem, wo: &World<T,I>*/
) -> Vec<Resume> {
    let mut out = Vec::new();
    let mut count = 0;
    let t = plan.len();
...
pub fn poidsparametredesavantage(
    poids: i32,
    support: &DMatrix<i32>,
    h: &HashMap<SymId, Vec<Op>>,
    plan: &Vec<Op>,
